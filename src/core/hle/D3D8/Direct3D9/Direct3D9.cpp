	...
	case xbox::X_D3DRTYPE_SURFACE: {
		...
		if (pParentXboxTexture && (pXboxSurface->Format == pParentXboxTexture->Format)) {
			...
			case D3DRTYPE_TEXTURE: {
				...
				if (hRet == D3D_OK) {
					SetHostSurface(pXboxSurface, pNewHostSurface.Get(), iTextureStage);
					// Mark surface entry as RT if created with RT usage
					if (D3DUsage & D3DUSAGE_RENDERTARGET) {
						auto srfKey = GetHostResourceKey(pXboxSurface, iTextureStage);
						auto& srfCache = GetResourceCache(srfKey);
						auto srfIt = srfCache.find(srfKey);
						if (srfIt != srfCache.end()) srfIt->second.isRenderTarget = true;
					}
